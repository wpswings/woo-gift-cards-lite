...
if( !class_exists( 'MWB_WGM_GIFTCARD_ACTIVATION_FUNCTION' ) )
{
	...
	class MWB_WGM_GIFTCARD_ACTIVATION_FUNCTION{
        ...
        public function mwb_wgm_restore_data() {
	        ...
	        if( $giftcard_enable == 'on' && isset( $giftcard_enable ) ){
	        ...
	        if( $mwb_check_enable ) {
	        	...
	        	if( $product_flag){
	        		$product_format_process_completion_flag = false;
	        		$product_format_flag = $this->restore_product_format_settings_data( $product_format_process_completion_flag);
	        	}
	        	if($product_format_flag){
	        ...
	        /*General setting tab data*/
	        function restore_general_settings_data( $general_process_completion_flag){	        	
		        $mwb_general_settings = get_option("mwb_wgm_general_settings", array());
		        $general_setting_flag = false;
		        if( !isset( $mwb_general_settings ) && empty( $mwb_general_settings ) ){
		        	$giftcard_enable = get_option("mwb_wgm_general_setting_enable", false);
			...
			function restore_product_settings_data( $product_process_completion_flag ){
				$product_setting_flag = false;
				$mwb_product_settings = get_option("mwb_wgm_product_settings", array());			
				if( !isset( $mwb_product_settings ) && empty($mwb_product_settings) ) {
			...
			/*Product setting tab format data*/
			function restore_product_format_settings_data( $product_format_process_completion_flag ){
				$product_setting_flag_for_category_format = false;
				$mwb_product_settings_format = get_option("mwb_wgm_product_setting_exclude_product_format", array());
				if( !isset($mwb_product_settings_format ) && empty( $mwb_product_settings_format ) ) {
					...
					$product_setting_flag_for_category_format = true;
				}
				
				if($product_setting_flag_for_category_format){
					delete_option('mwb_wgm_product_setting_exclude_product_format');
					$product_format_process_completion_flag = true;
					return $product_format_process_completion_flag;
				}
			}

			/*Email setting tab data*/
			function restore_mail_settings_data( $mail_process_completion_flag ){
				
				$mail_setting_flag = false;
				$mwb_mail_settings = get_option("mwb_wgm_mail_settings", array());
				if( !isset($mwb_mail_settings ) && empty( $mwb_mail_settings ) ) {
			...
			function restore_delivery_settings_data( $delivery_process_completion_flag){
				$delivery_setting_flag = false;
				$mwb_delivery_settings = get_option("mwb_wgm_delivery_setting_method", array());
				if( !isset($mwb_delivery_settings ) && empty( $mwb_delivery_settings ) ) {
			...
			function restore_other_settings_data( $other_process_completion_flag ){
				$other_setting_flag = false;
				$mwb_other_settings = get_option("mwb_wgm_other_settings", array());
				if( !isset($mwb_other_settings ) && empty( $mwb_other_settings ) ) {				
			...
			function delete_additional_data(){
