...
class Woocommerce_gift_cards_lite_Admin {
		...
		if ( $giftcard_enable ) {
			...
			if ( empty( $is_customizable ) ) {
				...
				<p class="form-field mwb_wgm_email_defualt_template">
					<label class = "mwb_wgm_email_defualt_template" for="mwb_wgm_email_defualt_template"><?php esc_html_e( 'Which template you want to be selected by default?', 'woocommerce_gift_cards_lite' ); ?></label>
					...
					<select id="mwb_wgm_email_defualt_template" name = "mwb_wgm_email_defualt_template" style="width: 50%">
						...
						if ( empty( $default_selected ) ) {
							?>
							<option value=""><?php esc_html_e( 'Select the template from above field ', 'woocommerce_gift_cards_lite' ); ?></option>
							<?php
		...
		if ( isset( $post->ID ) ) {
			...
			if ( isset( $product ) && is_object( $product ) ) {
				if ( $product->get_type() == 'wgm_gift_card' ) {
					check_admin_referer( 'mwb_wgm_lite_nonce', 'mwb_wgm_product_nonce_field' );
					...
					if ( $selected_pricing ) {
						...
						if ( ! isset( $_POST['mwb_wgm_email_template'] ) || empty( $_POST['mwb_wgm_email_template'] ) ) {
							...
							$mwb_wgm_pricing['template'] = array_map( 'sanitize_text_field', wp_unslash( $_POST['mwb_wgm_email_template'] ) );
						}
	...
	public function mwb_wgm_new_template() {
		...
		if ( empty( $mwb_wgm_template ) ) {
			...
			$gifttemplate_new = array(
				'post_title' => __( 'New Birthday Template', 'woocommerce_gift_cards_lite' ),
		...
		if ( isset( $_GET['mwb_wgm_template'] ) ) {
			if ( isset( $_GET['mwb_wgm_template'] ) == 'giftcard' ) {
				...
				if ( $expiry_date > 0 || $expiry_date === 0 ) {
					...
					$expirydate_format = esc_html__( 'No Expiration', 'woocommerce_gift_cards_lite' );
				}
	...
	public function mwb_wgm_append_default_template() {
		...
		$template_ids = isset( $_POST['template_ids'] ) ? sanitize_text_field( wp_unslash( $_POST['template_ids'] ) ) : '';
